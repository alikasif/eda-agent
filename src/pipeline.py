...
@function_tool
async def get_dataset_info(ctx: RunContextWrapper[ExecutorContext]) -> str:
...
@function_tool
async def execute_python(ctx: RunContextWrapper[ExecutorContext], code: str) -> str:
    ...
    result = await asyncio.to_thread(_tools.execute_python, code, ctx.context.df)
...
def stage_plan(state: EDAState, cb: Callable, emit: Callable) -> None:
    ...
    plan = call_llm_json(
        messages=messages,
        model=state.models.planner,
        api_base=state.models.api_base,
...
def _execute_hypothesis(
    ...
    # Pass emit into context — execute_python streams each artifact immediately
...
def stage_synthesize(state: EDAState, cb: Callable, emit: Callable) -> None:
    ...
    synthesis = call_llm_json(
        messages=messages,
        model=state.models.synthesizer,
        api_base=state.models.api_base,
...
def run_pipeline(
    ...
    executor_agent = Agent(
        name="EDA Executor",
        model=get_model(state.models.executor, api_base=state.models.api_base),

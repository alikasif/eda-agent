...
@function_tool
async def get_dataset_info(ctx: RunContextWrapper[EDAContext]) -> str:
...
@function_tool
async def execute_python(ctx: RunContextWrapper[EDAContext], code: str) -> str:
...
class EDAAgent:
    ...
    def __init__(self, models: ModelConfig):
        self.agent = Agent(
            name="EDA Agent",
            model=get_model(models.eda_agent, api_base=models.api_base),
